...
async def get_cards_list() -> list[dict[str, Any]]:
    ...
    async with database.session() as ses:
        return [
            {
                "card": {"card_id": row.card_id, "question": row.question,
                         "answer": row.answer, "added_at": row.added_at},
                "note": {"note_id": row.note_id, "material_title": row.title,
                         "content": row.content, "page": row.page, "chapter": row.chapter}
            }
